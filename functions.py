...
def overview_data(df, display_frst_value=False):
    ...
    if display_frst_value:
        ...
        first_value = pd.DataFrame(columns=["First Value"])
...
def plot_confusion_matrix(
    ...
    labels = np.empty_like(cm, dtype="<U16")

    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            count_label = f"Count:\n{cm[i, j]:0.0f}" if count else ""
            percent_label = f"{cm[i, j] / np.sum(cm[i, :]):.2%}" if percent else ""
            label_separator = "\n" if count and percent else ""

            label = (
                f"{group_names[i]}\n{count_label}{label_separator}{percent_label}"
                if group_names is not None
                else f"{count_label}{label_separator}{percent_label}"
            )
            labels[i, j] = label

    masked_labels = np.ma.masked_where(cm == 0, labels)
